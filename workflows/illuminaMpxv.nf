...
workflow prepareReferenceFiles {
  ...
  if (!params.custom_scheme) {
    ...
    ch_scheme = ch_trimmed_reads
      .map { sample, _forward, _reverse -> sample }
      .combine(scheme)
  }
...
workflow sequenceAnalysis {
  ...
  readTrimming.out
    .combine(prepareReferenceFiles.out, by: 0)
    .set { ch_toMap }
  ...
  ch_qc = align_trim.out.ptrimmed_bam.combine(callConsensusFreebayes.out.consensus, by: 0)
  ...
  callConsensusFreebayes.out.consensus
    .map { sampleName, sampleFasta -> sampleFasta }
